...
const errors = [];
const allowedHosts = new Set(['weltkern.com', 'www.weltkern.com', 'rajputrajesh-4489b.web.app']);
...
for (const [name, record] of Object.entries(records)) {
  ...
  if (!license || license.status !== 'pending') errors.push(`${name}: this staged web-evidence batch intentionally requires license.status=pending.`);
  ...
  for (const flag of ['weightsVerified', 'variableVerified', 'scriptsVerified']) {
    if (technical[flag] !== false) errors.push(`${name}: staged identity-only record must keep technical.${flag}=false until separate technical evidence exists.`);
  }
}

console.log(`Independent web identity evidence: ${Object.keys(records).length} families; all licenses/technical fields intentionally pending.`);
if (errors.length) {
  console.error(`Errors: ${errors.length}`);
  errors.forEach(error => console.error(`  ERROR ${error}`));
  process.exit(1);
}
console.log('Independent web identity evidence validation passed.');
