...
function AppContent() {
  ...
  useEffect(() => {
    ...
    if (!isReturningToCatalog) window.scrollTo(0, 0);
    prevPathRef.current = currentPath;

    if (window.ym) window.ym(105600326, 'hit', `${location.pathname}${location.search}`);
  }, [location.pathname, location.search]);
  ...
  useEffect(() => {
    document.documentElement.classList.toggle('invert-mode', isInverted);
  }, [isInverted]);
  ...
  const toggleCompare = useCallback((id: string) => {
    setCompareList(prev => {
      ...
      if (isComparing) return prev.filter(c => c !== id);
      if (prev.length >= 3) {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] font-sans text-neutral-800 selection:bg-blue-100">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:z-[100] focus:top-2 focus:left-2 focus:px-4 focus:py-2 focus:bg-neutral-800 focus:text-white focus:font-mono focus:text-xs focus:uppercase focus:tracking-widest">Skip to content</a>
      <Header
      ...
      <main id="main-content" className="min-h-screen">
        <AnimatePresence mode="wait">
          <Routes location={location} key={location.pathname}>
            ...
            <Route path="/compare" element={<PageTransition><ComparePage fonts={mockFonts.filter(f => compareList.includes(f.id))} allFonts={mockFonts} previewText={previewText} setPreviewText={setPreviewText} removeFromCompare={toggleCompare} toggleCompare={toggleCompare} onBack={() => navigate("/")} /></PageTransition>} />
            <Route path="/favorites" element={<PageTransition><FavoritesPage fonts={mockFonts.filter(f => favorites.includes(f.id))} previewText={previewText} setPreviewText={setPreviewText} toggleFavorite={toggleFavorite} compareList={compareList} toggleCompare={toggleCompare} viewDetails={viewDetails} onGoToCatalog={() => navigate("/")} /></PageTransition>} />
...
const FontDetailsWrapper = ({ mockFonts: fontList, ...props }: { mockFonts: Font[]; [key: string]: any }) => {
  ...
  if (!font) return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-8 bg-white">
      ...
      <p className="font-mono text-xs uppercase tracking-[0.2em] text-neutral-500 mb-12 max-w-md leading-relaxed">{language === 'ru' ? 'Запрошенная гарнитура не найдена в индексе.' : 'The requested typeface could not be located in the index.'}</p>
      <button type="button" onClick={() => navigate("/")} className="px-12 py-5 bg-neutral-800 text-white font-mono text-xs uppercase tracking-widest hover:bg-neutral-700 transition-colors">{t('details.back')}</button>
    </div>
  );
