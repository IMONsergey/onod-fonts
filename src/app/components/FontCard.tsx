...
export const FontCard: React.FC<FontCardProps> = memo(({
  ...
  const effectiveFamilyName = getEffectiveFamilyName(font);
  const effectiveCssStack = getEffectiveCssStack(font);
  ...
  const effectiveSourceLabel = getEffectiveSourceLabel(font);
  ...
  const showLegacyDownload = Boolean(font.downloadUrl && trust.identityVerified && trust.licenseVerified && trust.provider !== "Fontshare" && trust.provider !== "Independent");

  const style = {
    fontFamily: effectiveCssStack,
  ...
  const copyCss = async (e: React.MouseEvent) => {
    e.stopPropagation();
    const ok = await copyToClipboard(`font-family: ${effectiveCssStack};`);
    if (ok) {
      setCssCopied(true);
      ...
      <article className="group border-b border-neutral-200 bg-white min-h-[300px] flex flex-col md:flex-row transition-colors hover:bg-neutral-50">
        <div className="w-full md:w-72 flex-shrink-0 p-4 md:p-6 border-r border-neutral-200 flex flex-col justify-between relative">
          <div>
            ...
            <button type="button" className="block text-left text-xl md:text-3xl tracking-tighter leading-none mb-1 hover:underline decoration-2 underline-offset-4" onClick={() => onViewDetails(font.id)} style={{ fontWeight: 700 }}>{effectiveFamilyName}</button>
            <p className="font-mono text-[10px] text-neutral-500 uppercase tracking-wide mb-1">{effectiveAuthor}</p>
            <p className="font-mono text-[9px] text-neutral-400 uppercase tracking-wide mb-6">{effectiveSourceLabel}</p>
            ...
            <div className={cn("space-y-6 transition-opacity duration-300", runtime.status === "error" ? "opacity-30" : "opacity-20 group-hover:opacity-100")}>
              {hasWeightControls && (
                <div className="space-y-2">
                  ...
                  <input type="range" min={Number(effectiveWeights[0] || 100)} max={Number(effectiveWeights[effectiveWeights.length - 1] || 900)} step="100" value={localWeight} disabled={runtime.status === "error"} onChange={(e) => setLocalWeight(Number(e.target.value))} aria-label={`${effectiveFamilyName} weight`} className="w-full h-px bg-black appearance-none cursor-pointer accent-black disabled:cursor-not-allowed" />
                </div>
              )}
