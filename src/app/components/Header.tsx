...
import { Menu, Heart, X, type LucideIcon, Info, LayoutGrid, Moon, Sun, Globe, BookOpen, Layers } from "lucide-react";
...
export const Header: React.FC<HeaderProps> = ({
  ...
  // Close mobile menu on route change (activePage change)
  React.useEffect(() => {
  ...
  const NavItem = ({ page, label, icon: Icon, count }: { page: string; label: string; icon?: LucideIcon; count?: number }) => (
    <button
      ...
      className={cn(
        "h-full px-5 flex items-center gap-2 text-[12px] uppercase tracking-[0.12em] transition-all duration-200",
        activePage === page
          ? "text-neutral-800 bg-neutral-100"
          : "text-neutral-500 hover:text-neutral-800 hover:bg-neutral-50"
      )}
      style={{ fontWeight: activePage === page ? 600 : 450 }}
    >
      ...
      {count !== undefined && count > 0 && (
        <span className={cn(
            ...
            activePage === page ? "bg-neutral-200 text-neutral-700" : "bg-neutral-100 text-neutral-500"
    ...
    <header className="sticky top-0 z-[60] w-full bg-white/90 backdrop-blur-lg border-b border-neutral-200/60">
      <div className="flex h-16 items-stretch justify-between max-w-[1800px] mx-auto">
        ...
        <div className="flex items-center">
             <div 
                className="flex items-center px-6 cursor-pointer hover:bg-neutral-50 transition-colors h-full" 
                onClick={() => navigate("/")}
             >
                <div className="flex items-center text-neutral-800">
                    <svg width="100" height="20" viewBox="0 0 215 42" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <nav className="hidden md:flex items-stretch h-full ml-auto">
          {/* Primary: Catalog / Compare / Favorites */}
          ...
          {/* Divider */}
          <div className="w-px bg-neutral-200/60 my-3" />
          
          {/* Secondary: Protocol / About */}
          ...
          <button 
             ...
             className="h-full px-4 flex items-center gap-1.5 text-[12px] uppercase tracking-[0.12em] text-neutral-400 hover:text-neutral-800 hover:bg-neutral-50 transition-all"
          ...
          <button 
             ...
             className={cn(
                 "h-full px-4 flex items-center text-[12px] transition-all",
                 isInverted ? "bg-neutral-800 text-white hover:bg-neutral-700" : "text-neutral-400 hover:text-neutral-800 hover:bg-neutral-50"
             )}
             title="Invert Mode"
        ...
        <div className="flex md:hidden">
            <button 
                className={cn(
                  "px-4 flex items-center justify-center gap-1.5 transition-all relative",
            ...
            >
                ...
                {compareCount > 0 && (
                  <span className="absolute top-3 right-2.5 min-w-[16px] h-[16px] flex items-center justify-center px-1 text-[9px] rounded-full bg-neutral-800 text-white" style={{ fontWeight: 600 }}>
                    {compareCount}
                  </span>
                )}
            </button>
            ...
            >
                <Heart className={cn("w-5 h-5", favoritesCount > 0 && "fill-current")} />
                {favoritesCount > 0 && (
                  <span className="absolute top-3 right-2.5 min-w-[16px] h-[16px] flex items-center justify-center px-1 text-[9px] rounded-full bg-neutral-800 text-white" style={{ fontWeight: 600 }}>
                    {favoritesCount}
                  </span>
            ...
            <button
                className="px-5 text-neutral-500 hover:text-neutral-800 hover:bg-neutral-50 transition-all flex items-center justify-center"
      ...
      {/* Mobile Nav Drawer — rendered via portal to escape backdrop-blur containing block */}
      {createPortal(
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
            ...
            >
              ...
              <div className="relative h-full flex flex-col justify-between overflow-y-auto">
                {/* Watermark */}
                <div 
                  className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[40vw] leading-none tracking-tighter text-neutral-100 pointer-events-none select-none"
                ...
                {/* Nav Items */}
                <nav className="relative z-10 flex flex-col pt-4">
                  {[
                    ...
                    return (
                      <motion.button
                        ...
                        className={cn(
                          "group relative text-left transition-colors duration-300",
                      ...
                      >
                        ...
                        <div className={cn(
                          ...
                          isActive ? "bg-neutral-800" : "bg-neutral-200"
                        )} />
                        ...
                        <div className="flex items-baseline justify-between px-6 py-5">
                          ...
                          <div className="flex items-baseline gap-4 overflow-hidden">
                            ...
                            <span
                              className="text-[11vw] leading-[0.85] tracking-[-0.04em] uppercase block"
                          ...
                          {item.count !== undefined && item.count > 0 && (
                            <span 
                              className={cn(
                                "font-mono text-[11px] tracking-widest ml-3 shrink-0",
                ...
                >
                  ...
                  <div className="flex gap-3 mb-6">
                    <button 
                      onClick={() => setLanguage(language === 'en' ? 'ru' : 'en')}
                      className="flex-1 py-3 border border-neutral-200 text-neutral-500 hover:text-neutral-800 hover:border-neutral-400 transition-all font-mono text-[11px] uppercase tracking-[0.15em]"
                    >
                    ...
                    <button 
                      onClick={toggleInvert}
                      className={cn(
                        "flex-1 py-3 border transition-all font-mono text-[11px] uppercase tracking-[0.15em]",
                        isInverted 
                          ? "bg-neutral-900 text-white border-neutral-900" 
                          : "border-neutral-200 text-neutral-500 hover:text-neutral-800 hover:border-neutral-400"
                      )}
                    >
                      {isInverted 
                  ...
                  <div className="flex justify-between items-end">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-neutral-300">
                        {language === 'ru' ? 'Открытый дизайн-инструмент' : 'Open design instrument'}
                      </span>
                    </div>
