      ...
      <div className="sticky top-16 z-40 bg-white border-b border-neutral-200">
        <div className="flex flex-row items-stretch h-14 md:h-auto">
          <div className="flex items-center w-[42%] md:w-72 flex-shrink-0 border-r border-neutral-200 p-2 md:p-4">
            ...
            <div className="relative flex-grow min-w-0"><Search className="absolute left-0 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-neutral-400" /><input type="search" aria-label={t('search.placeholder')} placeholder={t('search.placeholder').toUpperCase()} value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))} className="h-8 md:h-10 w-full bg-transparent border-b border-transparent focus:border-neutral-400 pl-5 pr-1 text-[10px] md:text-sm font-mono uppercase placeholder:text-neutral-400 focus:outline-none transition-all truncate" /></div>
          </div>

          <div className="relative flex-grow md:border-r border-neutral-200 p-2 md:p-4 flex items-center gap-3 min-w-0">
            <input type="text" aria-label={t('preview.placeholder')} placeholder={t('preview.placeholder').toUpperCase()} value={previewText} onChange={(e) => setPreviewText(e.target.value)} className="h-8 md:h-10 flex-grow bg-transparent text-sm md:text-2xl font-light placeholder:text-neutral-200 focus:outline-none min-w-0" />
