          ...
          <div className="border-b border-neutral-200 px-4 md:px-6 py-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex flex-wrap gap-2 items-center min-h-[24px]">
                {filters.search && <button type="button" onClick={() => setFilters(prev => ({ ...prev, search: "" }))} className="flex items-center gap-1 border border-neutral-300 px-2.5 py-1 font-mono text-[10px] uppercase hover:border-neutral-400 hover:text-neutral-500">Q: {filters.search.slice(0, 22)} <X className="w-3 h-3" /></button>}
                {filters.categories.map(cat => <button type="button" key={`cat-${cat}`} onClick={() => removeCategory(cat)} className="flex items-center gap-1 bg-neutral-800 text-white px-2.5 py-1 font-mono text-[10px] uppercase hover:bg-neutral-500">{cat} <X className="w-3 h-3" /></button>)}
                {filters.languages.map(lang => <button type="button" key={`lang-${lang}`} onClick={() => removeLanguage(lang)} className="flex items-center gap-1 border border-neutral-300 px-2.5 py-1 font-mono text-[10px] uppercase hover:border-neutral-400 hover:text-neutral-500">{lang} <X className="w-3 h-3" /></button>)}
                {filters.sources.map(source => <button type="button" key={`source-${source}`} onClick={() => removeSource(source)} className="flex items-center gap-1 border border-neutral-300 px-2.5 py-1 font-mono text-[10px] uppercase hover:border-neutral-400 hover:text-neutral-500 max-w-48"><span className="truncate">{source}</span><X className="w-3 h-3 shrink-0" /></button>)}
                {filters.licenses.map(license => <button type="button" key={`license-${license}`} onClick={() => removeLicense(license)} className="flex items-center gap-1 border border-neutral-300 px-2.5 py-1 font-mono text-[10px] uppercase hover:border-neutral-400 hover:text-neutral-500">{license} <X className="w-3 h-3" /></button>)}
                {filters.variableOnly && <button type="button" onClick={() => setFilters(prev => ({ ...prev, variableOnly: false }))} className="flex items-center gap-1 bg-neutral-200 px-2.5 py-1 font-mono text-[10px] uppercase hover:bg-neutral-500 hover:text-white">VAR ONLY <X className="w-3 h-3" /></button>}
                {filters.minWeights && <button type="button" onClick={() => setFilters(prev => ({ ...prev, minWeights: undefined }))} className="flex items-center gap-1 bg-neutral-200 px-2.5 py-1 font-mono text-[10px] uppercase hover:bg-neutral-500 hover:text-white">{filters.minWeights}+ WEIGHTS <X className="w-3 h-3" /></button>}
