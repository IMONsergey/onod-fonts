  ...
  return <div className="min-h-screen bg-white text-black">
    ...
    <section className="grid grid-cols-1 lg:grid-cols-[320px_1fr] border-b border-neutral-200">
      <aside className="p-6 md:p-8 bg-neutral-50 border-b lg:border-b-0 lg:border-r border-neutral-200">
        <div className="flex items-center justify-between gap-3 mb-5"><span className="font-mono text-[9px] uppercase text-neutral-400">TYPEFACE / {font.source}</span><span title={trust.verificationSource} className={cn("font-mono text-[8px] uppercase border px-1.5 py-1", trust.upstreamVerified ? "border-emerald-300 text-emerald-700 bg-emerald-50" : trust.confidence === "derived" ? "border-amber-300 text-amber-700 bg-amber-50" : "border-neutral-300 text-neutral-500")}>{trust.upstreamVerified ? "UPSTREAM VERIFIED" : trust.confidence === "derived" ? "DATA: DERIVED" : "DATA: CURATED"}</span></div>
      ...
      <div className="min-w-0">
        ...
        {tab === "about" && <div role="tabpanel" className="p-6 md:p-12 grid xl:grid-cols-[1fr_360px] gap-12"><div><p className="text-3xl md:text-5xl tracking-tight leading-[1.05] mb-12">{font.description}</p><div className="font-mono text-[9px] uppercase text-neutral-400 mb-2">{language === "ru" ? "ЕЩЁ В КАТЕГОРИИ" : "MORE IN CATEGORY"}</div><p className="font-mono text-[9px] text-neutral-400 mb-5">{language === "ru" ? "Не семантические рекомендации: только та же широкая категория." : "Not semantic recommendations: same broad category only."}</p><div className="grid sm:grid-cols-2 gap-px bg-neutral-200">{related.map(item => <div key={item.id} className="bg-white p-4"><div className="text-2xl" style={{ fontFamily: item.cssStack }}>{item.name}</div><div className="font-mono text-[9px] text-neutral-400 mt-2">{getEffectiveAuthor(item)}</div></div>)}</div></div><div><div className="font-mono text-[9px] uppercase text-neutral-400 mb-5">CODE</div><div className="space-y-3">{Object.entries(snippets).map(([key,value]) => <div key={key} className="border border-neutral-200"><div className="flex justify-between px-3 py-2 border-b border-neutral-200 font-mono text-[9px] uppercase"><span>{key}</span><button type="button" onClick={() => copy(key,value)} aria-label={`Copy ${key}`}>{copied === key ? <Check className="w-3.5 h-3.5"/> : <Code className="w-3.5 h-3.5"/>}</button></div><pre className="p-3 overflow-auto text-[10px] whitespace-pre-wrap break-all">{value}</pre></div>)}</div></div></div>}
      </div>
    </section>
  </div>;
};
