...
export const FontDetailsPage: React.FC<FontDetailsProps> = ({ font, onBack, toggleFavorite, isFavorite, toggleCompare, isCompare, testPairing, previewText }) => {
  ...
  const snippets = useMemo(() => ({
    import: font.source === "Google Fonts" ? `@import url('${googleImportUrl(font)}');` : font.customCssUrl ? `@import url('${font.customCssUrl}');` : `/* Load ${font.name} from ${font.source} */`,
  ...
  const metricValue = trust.confidence === "derived" ? (language === "ru" ? "Не подтверждено" : "Unverified") : String(weights.length);
  ...
  return <div className="min-h-screen bg-white text-black">
    ...
    <div className="sticky top-16 z-50 h-16 bg-white border-b border-neutral-200 flex items-center">
      ...
      {font.downloadUrl && <button type="button" onClick={() => open(font.downloadUrl!)} className="h-full px-5 border-l border-neutral-200 hidden sm:flex items-center gap-2 font-mono text-[10px] uppercase"><Download className="w-3.5 h-3.5"/>{t("details.download")}</button>}
      <button type="button" onClick={() => open(font.sourceUrl)} className="h-full px-6 bg-neutral-900 text-white hidden sm:flex items-center gap-2 font-mono text-[10px] uppercase"><ExternalLink className="w-3.5 h-3.5"/>{language === "ru" ? "Источник" : "Source"}</button>
    </div>
    ...
    <section className="grid grid-cols-1 lg:grid-cols-[320px_1fr] border-b border-neutral-200">
      <aside className="p-6 md:p-8 bg-neutral-50 border-b lg:border-b-0 lg:border-r border-neutral-200">
        <div className="flex items-center justify-between gap-3 mb-5"><span className="font-mono text-[9px] uppercase text-neutral-400">TYPEFACE / {font.source}</span><span className={cn("font-mono text-[8px] uppercase border px-1.5 py-1", trust.confidence === "derived" ? "border-amber-300 text-amber-700 bg-amber-50" : "border-neutral-300 text-neutral-500")}>{trust.confidence === "derived" ? "DATA: DERIVED" : "DATA: CURATED"}</span></div>
        ...
        {pairing && <div className="mt-10 border border-neutral-200 bg-white p-4"><div className="font-mono text-[9px] uppercase text-neutral-400 mb-3">{language === "ru" ? "Контрастное сочетание" : "Contrast suggestion"}</div><div className="text-2xl mb-1" style={{ fontFamily: pairing.cssStack }}>{pairing.name}</div><p className="font-mono text-[8px] text-neutral-400 mb-4">{language === "ru" ? "Эвристика категории, не метрический анализ." : "Category heuristic, not metric analysis."}</p>{testPairing && <button type="button" onClick={() => testPairing([font.id, pairing.id])} className="w-full border border-neutral-300 py-2 font-mono text-[9px] uppercase">{language === "ru" ? "Открыть" : "Open pairing"}</button>}</div>}
      </aside>
      ...
      <div className="min-w-0">
        ...
        {tab === "specimen" && <div role="tabpanel">
          <div className="p-4 border-b border-neutral-200 flex flex-wrap gap-5 items-center bg-neutral-50/50"><Control label={t("details.size")} value={size} min={16} max={180} step={1} onChange={setSize}/><Control label={t("details.line")} value={lineHeight} min={0.8} max={2.2} step={0.1} onChange={setLineHeight}/><Control label={t("details.track")} value={tracking} min={-0.1} max={0.4} step={0.01} onChange={setTracking}/><select aria-label="Weight" value={weight} disabled={runtime.status === "error" || weights.length < 2} onChange={e => setWeight(e.target.value)} className="h-9 border border-neutral-300 bg-white px-3 font-mono text-[10px] uppercase disabled:opacity-40">{weights.map(item => <option key={item}>{item}</option>)}</select>{trust.confidence === "derived" && <span className="font-mono text-[8px] uppercase text-amber-700 border border-amber-200 bg-amber-50 px-2 py-1">400 ONLY / VERIFY METRICS</span>}<div className="flex border border-neutral-300 ml-auto"><IconButton label="Align left" active={align === "left"} onClick={() => setAlign("left")}><AlignLeft className="w-3.5 h-3.5"/></IconButton><IconButton label="Align center" active={align === "center"} onClick={() => setAlign("center")}><AlignCenter className="w-3.5 h-3.5"/></IconButton><IconButton label="Align right" active={align === "right"} onClick={() => setAlign("right")}><AlignRight className="w-3.5 h-3.5"/></IconButton></div></div>
          <div className="p-6 md:p-12 min-h-[55vh]"><textarea aria-label={`${font.name} specimen`} value={text} onChange={e => setText(e.target.value)} className="w-full min-h-[40vh] resize-none bg-transparent outline-none" style={{ fontFamily: font.cssStack, fontSize: `${size}px`, lineHeight, letterSpacing: `${tracking}em`, textAlign: align, fontWeight: weight }}/></div>
          <div className="border-t border-neutral-200 p-6 md:p-12"><div className="font-mono text-[10px] uppercase text-neutral-400 mb-8">{trust.confidence === "derived" ? "VERIFIED PREVIEW WEIGHTS" : t("details.allWeights")}</div><div className="space-y-8">{weights.map(item => <button type="button" key={item} onClick={() => setWeight(item)} className="w-full text-left"><span className="font-mono text-[9px] text-neutral-400">{item}</span><div className="text-4xl md:text-6xl leading-none" style={{ fontFamily: font.cssStack, fontWeight: item }}>{font.name} {item}</div></button>)}</div></div>
        </div>}

        {tab === "glyphs" && <div role="tabpanel" className="p-6 md:p-12"><p className="mb-6 font-mono text-[9px] uppercase text-neutral-400">{language === "ru" ? "Демо-набор символов, не подтверждённая карта покрытия." : "Sample character set, not a verified coverage map."}</p><div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 xl:grid-cols-12 border-l border-t border-neutral-200">{[...chars].map((char,i) => <div key={`${char}-${i}`} className="aspect-square border-r border-b border-neutral-200 flex items-center justify-center text-2xl" style={{ fontFamily: font.cssStack, fontWeight: weight }}>{char}</div>)}</div></div>}

        {tab === "about" && <div role="tabpanel" className="p-6 md:p-12 grid xl:grid-cols-[1fr_360px] gap-12"><div><p className="text-3xl md:text-5xl tracking-tight leading-[1.05] mb-12">{font.description}</p><div className="font-mono text-[9px] uppercase text-neutral-400 mb-2">{language === "ru" ? "ЕЩЁ В КАТЕГОРИИ" : "MORE IN CATEGORY"}</div><p className="font-mono text-[9px] text-neutral-400 mb-5">{language === "ru" ? "Не семантические рекомендации: только та же широкая категория." : "Not semantic recommendations: same broad category only."}</p><div className="grid sm:grid-cols-2 gap-px bg-neutral-200">{related.map(item => <div key={item.id} className="bg-white p-4"><div className="text-2xl" style={{ fontFamily: item.cssStack }}>{item.name}</div><div className="font-mono text-[9px] text-neutral-400 mt-2">{item.author}</div></div>)}</div></div><div><div className="font-mono text-[9px] uppercase text-neutral-400 mb-5">CODE</div><div className="space-y-3">{Object.entries(snippets).map(([key,value]) => <div key={key} className="border border-neutral-200"><div className="flex justify-between px-3 py-2 border-b border-neutral-200 font-mono text-[9px] uppercase"><span>{key}</span><button type="button" onClick={() => copy(key,value)} aria-label={`Copy ${key}`}>{copied === key ? <Check className="w-3.5 h-3.5"/> : <Code className="w-3.5 h-3.5"/>}</button></div><pre className="p-3 overflow-auto text-[10px] whitespace-pre-wrap break-all">{value}</pre></div>)}</div></div></div>}
      </div>
    </section>
  </div>;
};
